...
export class Customer {
  ...
  private validateName(name: string): void {
    ...
    if (trimmed.length < 2 || trimmed.length > 80) {
      throw new Error('Nome do cliente deve ter entre 2 e 80 caracteres');
    }
  }
  ...
  private validatePhone(phone: string): void {
    ...
    if (digits.length < 10 || digits.length > 11) {
      throw new Error('Telefone do cliente deve ter 10 ou 11 dígitos');
    }

    if (/^(\d)\1+$/.test(digits)) {
      throw new Error('Telefone do cliente não pode ser uma sequência repetida de dígitos');
    }

    const ddd = parseInt(digits.substring(0, 2), 10);
    if (ddd < 11 || ddd > 99) {
      throw new Error('DDD do telefone do cliente é inválido');
    }
